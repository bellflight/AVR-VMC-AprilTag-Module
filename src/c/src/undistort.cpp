...
// Camera intrinsic parameters, initially identity (will be estimated by
// calibration process).
...
// Initialize the fisheye lens model with the coefficients given by calibration
// procedure.
...
void setup_vpi(cv::Mat img_rgba8) {
  ...
  // Fill up the camera intrinsic parameters given by camera calibration
  // procedure.
...

{
  ...
  CHECK_STATUS(vpiSubmitRemap(stream, VPI_BACKEND_CUDA, remap, tmpIn, tmpOut,
                              VPI_INTERP_CATMULL_ROM, VPI_BORDER_ZERO, 0));

  // Convert the result NV12 back to BGR, writing back to the input image.
  CHECK_STATUS(vpiSubmitConvertImageFormat(stream, VPI_BACKEND_CUDA, tmpOut,
                                           vimg, NULL));
